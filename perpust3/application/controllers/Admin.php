...
//proses upload Gambar
	...
	function peminjaman(){
		...
		$data['peminjaman'] = $this->db->query("SELECT * FROM transaksi T, buku B, anggota A WHERE T.id_buku=B.id_buku and
	...
	function transaksi_selesai($id){
		$data['buku'] = $this->M_perpus->get_data('buku')->result();
		$data['anggota'] = $this->M_perpus->get_data('anggota')->result();
		$data['peminjaman'] = $this->db->query("select * from peminjaman p, detail_pinjam d,anggota a, buku b where t.id_buku = b.id_buku and
...
//update status Buku
			...
			$data['buku'] = $this->M_perpus->get_data('buku')->result();
			$data['anggota'] = $this->M_perpus->get_data('anggota')->result();
			$data['peminjaman'] = $this->db->query("select * from peminjaman p, anggota a, detail_pinjam d, buku b where p.id_anggota = a.id_anggota and
...
function laporan_transaksi(){
	...
	if($this->form_validation->run() != false){
		$data['laporan'] = $this->db->query("select * from peminjaman p, detail_pinjam d, buku b, anggota a where p.id_pinjam=d.id_pinjam and
			b.id_buku=d.id_buku and a.id_anggota=p.id_anggota

			and date(p.tgl_pinjam)>='$dari' and date(p.tgl_pinjam)<='$sampai'")->result();
		$this->load->view('admin/header');
